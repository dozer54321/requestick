    ...
    const child = spawn(
      ...
      [
        ...
        [
          ...
          "if [ -f /app/host-pack/docker-compose.host-caddy.yml ]; then cp -f /app/host-pack/docker-compose.host-caddy.yml /work/docker-compose.host-caddy.yml; fi",
          "if [ -f /app/host-pack/Caddyfile ]; then cp -f /app/host-pack/Caddyfile /work/Caddyfile; fi",
          "if grep -q '^MESH_PROXY=host' /work/mesh.env 2>/dev/null; then docker compose --env-file mesh.env -f docker-compose.yml -f docker-compose.host-caddy.yml -p mesh up -d --no-deps --force-recreate --remove-orphans app > /work/backups/update.log 2>&1; else docker compose --env-file mesh.env -p mesh up -d --no-deps --force-recreate --remove-orphans app > /work/backups/update.log 2>&1; fi",
        ].join(" && "),
      ],
